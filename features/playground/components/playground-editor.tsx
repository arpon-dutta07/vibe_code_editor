...
export const PlaygroundEditor = ({
            ...
            return {
                provideInlineCompletions: async (model: any, position: any, context: any, token: any) => {
                    ...
                    console.log("Providing inline completion", {
                        suggestionId: currentSuggestionRef.current?.id,
    ...
    useEffect(() => {
        ...
        if (suggestion && suggestionPosition) {
            // CRITICAL: Set currentSuggestionRef immediately so Tab key handler can access it
            ...
            // Clear suggestion when no longer available
            currentSuggestionRef.current = null
        }
        ...
        tabCommandRef.current = editor.addCommand(
            ...
            () => {
                ...
                if (suggestionAcceptedRef.current) {
                    console.log("BLOCKED: Suggestion was just accepted, using default tab behavior")
                    // Let Monaco's default Tab handler work
                    return
                }
                ...
                // Default tab behavior - insert spaces or indentation
                console.log("DEFAULT: Inserting default indentation")
                const position = editor.getPosition()
                const lineContent = editor.getModel().getLineContent(position.lineNumber)
                const indentation = "\t" // Use tab character (can be configured)

                editor.executeEdits("tab-command", [
                    {
